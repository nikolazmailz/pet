...
public class ResponseProcessingService {
    ...
    public static final String STATUS_ERROR_REPLY = "ERROR_REPLY";
    private final MessageCodecPort codec;
    private final FileStoragePort fileStorage;
    private final RequestPersistencePort persistence;
    private final int maxInnerMessageSizeKb;

    public ResponseProcessingService(MessageCodecPort codec, FileStoragePort fileStorage, RequestPersistencePort persistence, int maxInnerMessageSizeKb) {
        this.codec = codec;
        this.fileStorage = fileStorage;
        this.persistence = persistence;
        this.maxInnerMessageSizeKb = maxInnerMessageSizeKb;
    }
    ...
    public String success(PreparedRequest<?> prepared, Object payload) throws Exception {
        ...
        ResponseEnvelope<?> response = ResponseEnvelope.success(prepared.systemParams(), payload);
    ...
    public String error(SystemParams params, RequestRecord requestRecord, long startedAt, String code, String message) throws Exception {
        if (params == null) {
            ...
            params.setCorrelationId("unknown");
            params.setRequestType("unknown");
        }
        if (requestRecord != null && params.getCorrelationId() != null)
            persistence.updateRequestStatus(params.getCorrelationId(), STATUS_ERROR_REPLY);
    ...
    private String finish(SystemParams params, RequestRecord requestRecord, long startedAt, ResponseEnvelope<?> response, String code) throws Exception {
        ...
        if (requestRecord != null) {
            responseRecord = persistence.saveResponse(params.getCorrelationId(), requestRecord, fullMessage, processingTime, code);
        }
    ...
    private String checkMessageSize(SystemParams params, ResponseEnvelope<?> response, String message) throws Exception {
        ...
        String messageUuid = fileStorage.storeMessage(
                params.getAdLogin(),
                params.getSessionId(),
                params.getTraceId(),
                params.getAdLogin() + "_" + params.getRequestType(),
                message, params.getCorrelationId());

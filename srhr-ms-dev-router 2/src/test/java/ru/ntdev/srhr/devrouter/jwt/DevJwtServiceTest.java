...
class DevJwtServiceTest {
    ...
    @Test
    void usesBrowserSessionIdAsCtxiWhenProvidedByRouter() throws Exception {
        DevJwtService service = new DevJwtService(properties());

        IssuedDevToken issued = service.issue("developer", "browser-session-id", "127.0.0.1");
        SignedJWT parsed = SignedJWT.parse(issued.value());

        assertThat(parsed.getJWTClaimsSet().getStringClaim("ctxi"))
                .isEqualTo("browser-session-id");
    }
